...
//TODO to review the format of the Xml file format.
...
public abstract class ActionTestHelper extends TestCase
{
  ...
  protected static final String DATA_PATH        = "Enterprise/data/";
  ...
  protected void setUp() throws java.lang.Exception
  {
    try
    {
      ...
      _enterpriseMgr = ServiceLookupHelper.getEnterpriseHierarchyMgr();
      _userMgr = ServiceLookupHelper.getUserManager();
      _partnerMgr = ServiceLookupHelper.getPartnerManager();
      _sessionMgr = getSessionMgr();
      _sharedResMgr = getSharedResManager();
    ...
    SecurityInfo secInfo)
  {
    ...
    channel.setTptProtocolType("PROTOCOL_TYPE");
  ...
  protected CommInfo createCommInfo(String name, String desc) throws Exception
  {
    ...
    jmsInfo.setTptImplVersion("TPT_IMPL_VERSION");
    jmsInfo.setDestType(JMSCommInfo.TOPIC);
    jmsInfo.setDestination("DESTINATION");
    
    commInfo.setIsDefaultTpt(true);
    commInfo.setName(name);
    commInfo.setProtocolType(CommInfo.JMS);
    commInfo.setRefId("REF_ID");
    commInfo.setTptImplVersion("TPT_IMPL_VERSION");
    ...
    throws Exception
  {
    ...
    try
    {
      SecurityDBHandler.getInstance().insertCertificate(new Integer(id), name, filename);
...
syncCert.serialize("certFile"+id+".cer");
  ...
  protected ISharedResourceManagerObj getSharedResManager() throws Exception
  {
    ISharedResourceManagerHome sharedResHome =
      (ISharedResourceManagerHome)ServiceLocator.instance(
      ServiceLocator.CLIENT_CONTEXT).getHome(
      ISharedResourceManagerHome.class.getName(),
      ISharedResourceManagerHome.class);
    return sharedResHome.create();
  }
  ...
  protected void deleteCertificate(Long uid) throws Exception
  {
    ...
    _certMgr.deleteCertificate(cert.getID(), cert.getCertName());
  }
  ...
  protected Collection shareBe(Long[] beUIDs, String toEnterpriseID)
  {
    Collection sharedResUIDs = null;
    try
    {
      ...
      sharedResUIDs = _sharedResMgr.shareResourceIfNotShared(
                        IResourceTypes.BUSINESS_ENTITY, beList, toEnterpriseID);
    }
    catch (Exception ex)
    {
      Log.err("TEST", "[ActionTestHelper.shareBe] Error", ex);
      assertTrue("unable to shareBe", false);
    }
    finally
    {
      Log.debug("TEST", "[ActionTestHelper.shareBe] Exit ");
    }

    return sharedResUIDs;
  }

  protected SharedResource getSharedResource(Long sharedResUID)
  {
    SharedResource sharedRes = null;
    try
    {
      sharedRes = _sharedResMgr.getSharedResourceByUID(sharedResUID);
    }
    catch (Exception ex)
    {
      Log.err("TEST", "[ActionTestHelper.getSharedResource] Error", ex);
      assertTrue("unable to getSharedResource", false);
    }
    finally
    {
      Log.debug("TEST", "[ActionTestHelper.getSharedResource] Exit ");
    }
  ...
  protected void checkDeserCommInfo(CommInfo origin, CommInfo deserCi)
  {
    ...
    assertEquals("URL is incorrect", origin.getURL(), deserCi.getURL());
  }
  ...
  protected void checkDeserCert(SyncCertificate syncCert, Long certUID)
  {
    ...
    //assertEquals("Bytecontent incorrect", new String(syncCert.getCertBytes()), cert.getCertificate());
    assertEquals("Bytecontent incorrect", syncCert.getCert().getCertificate(), cert.getCertificate());
  }

  protected Collection wrapCollection(Object[] array)
  {
