 ...
 *
 * File: RemoveAccessRightActionTest.java
 *
 ****************************************************************************
 * Date           Author              Changes
 ****************************************************************************
...
/**
 * This test case tests the functionality of the RemoveAccessRightAction class.
 *
 * @author Neo Sok Lay
 *
 * @version 2.0
 * @since 2.0
 */
public class RemoveAccessRightActionTest extends TestCase
{
  private static final String CLASSNAME = "RemoveAccessRightActionTest";
  ...
  RemoveAccessRightEvent[] _events;
  StateMachine[]        _sm;
  String[]              _sessions;
  ...
  public RemoveAccessRightActionTest(String name)
  {
    super(name);
  }
  ...
  {
    return new TestSuite(RemoveAccessRightActionTest.class);
  }
...
//      lookupUserManager();
   ...
   try
    {
      ...
      prepareTestData();
      removeRejectedByAction();
      removeAcceptedByManager();
      cleanupTestData();
  ...
  private void removeRejectedByAction() throws Exception
  {
    // **  REJECTED BY ACTION

    // null event.
    checkRemoveAccessRightFail(null, _sessions[0], _sm[0], true);

    // invalid session id
    checkRemoveAccessRightFail(_events[0], "AAA", _sm[0], true);

    // not auth session
    checkRemoveAccessRightFail(_events[0], _sessions[1], _sm[0], true);

    // invalid auth subject (session name)
    checkRemoveAccessRightFail(_events[0], _sessions[0], _sm[1], true);

    // non existing AccessRight
    checkRemoveAccessRightFail(_events[1], _sessions[0], _sm[0], true);
  }

  private void removeAcceptedByManager() throws Exception
  {
    // *** ACCEPTED BY Manager
    // test remove
    checkRemoveAccessRightSuccess(_events[2], _sessions[0], _sm[0], ACR_UID_1);
  }

  private void checkRemoveAccessRightSuccess(
    ...
    Long acrUID)
  {
    ...
    try
    {
      response = performEvent(new RemoveAccessRightAction(), event, session, sm);
    }
    catch (Exception ex)
    {
      ACLLogger.errorLog(CLASSNAME, "checkRemoveAccessRightSuccess", "Error Exit", ex);
    ...
    checkAccessRightRemoved(acrUID);
  }

  private void checkAccessRightRemoved(Long acrUID)
  {
    AccessRight saved = getAccessRight(acrUID);
    assertNull("Access right not removed", saved);
  }

  private void checkRemoveAccessRightFail(
    IEvent event, String session, StateMachine sm, boolean eventEx)
  {
    ...
    {
      ACLLogger.infoLog(CLASSNAME, "checkRemoveAccessRightFail", "Fail due to event exception: "+ex.getMessage());
      if (!eventEx)
        assertTrue("Unexpected Event Exception", false);
      ACLLogger.infoLog(CLASSNAME, "checkRemoveAccessRightFail", "Exit");
      return;
    }
    catch (Exception ex)
    {
      ACLLogger.errorLog(CLASSNAME, "checkRemoveAccessRightFail", "Error Exit", ex);
      assertTrue("Event Exception", false);
    }
    assertNotNull("Response is null", response);
    assertTrue("Event status is incorrect", !response.isEventSuccessful());
...
//  }
  ...
  private void prepareTestData() throws Exception
  {
    ...
    _events = new RemoveAccessRightEvent[]
    {
  ...
  private RemoveAccessRightEvent createEvent(Long acrUId)
    throws Exception
  {
    return new RemoveAccessRightEvent(acrUId);
  }
