 ...
 *
 * File: GetAccessRightActionTest.java
 *
 ****************************************************************************
 * Date           Author              Changes
 ****************************************************************************
...
/**
 * This test case tests the functionality of the GetAccessRightAction class.
 *
 * @author Neo Sok Lay
 *
 * @version 2.0
 * @since 2.0
 */
public class GetAccessRightActionTest extends TestCase
{
  private static final String CLASSNAME = "GetAccessRightActionTest";
  ...
  private Long ACR_UID_1;


  public GetAccessRightActionTest(String name)
  {
    super(name);
  }
  ...
  {
    return new TestSuite(GetAccessRightActionTest.class);
  }
...
//      lookupUserManager();
  ...
  private void getRejectedByAction() throws Exception
  {
    ...
    // null event.
    checkGetAccessRightFail(null, _sessions[0], _sm[0], true);

    // invalid session id
    checkGetAccessRightFail(_events[0], "AAA", _sm[0], true);

    // not auth session
    checkGetAccessRightFail(_events[0], _sessions[1], _sm[0], true);

    // invalid auth subject (session name)
    checkGetAccessRightFail(_events[0], _sessions[0], _sm[1], true);
  }
  ...
  private void getAcceptedByManager() throws Exception
  {
    // *** ACCEPTED BY Manager
    // test get
    checkGetAccessRightSuccess(_events[2], _sessions[0], _sm[0], _mockAcrs[0]);
  }

  private void getRejectedByManager() throws Exception
  {
    // non existing AccessRight
    checkGetAccessRightFail(_events[1], _sessions[0], _sm[0], false);
  }

  private void checkGetAccessRightSuccess(
    ...
    AccessRight acr)
  {
    ...
    try
    {
      response = performEvent(new GetAccessRightAction(), event, session, sm);
    }
    catch (Exception ex)
    {
      ACLLogger.errorLog(CLASSNAME, "checkGetAccessRightSuccess", "Error Exit", ex);
    ...
    assertTrue("ReturnData not of correct type", returnedData instanceof Map);

    checkAccessRightRetrieved(acr, (Map)returnedData);
  }

  private void checkAccessRightRetrieved(AccessRight mock, Map returnedData)
  {
    assertEquals("ACR UID is incorrect", new Long(mock.getUId()), returnedData.get(AccessRight.UID));
    assertEquals("Action is different", mock.getAction(), returnedData.get(AccessRight.ACTION));
    if (mock.getCriteria() == null)
      assertNull("Criteria should be null", returnedData.get(AccessRight.CRITERIA));
    else
    {
      assertNotNull("Criteria should not be null", returnedData.get(AccessRight.CRITERIA));
      assertEquals("Critieria is different", mock.getCriteria().getFilterExpr(),
        ((DataFilterImpl)returnedData.get(AccessRight.CRITERIA)).getFilterExpr());
    }
    assertEquals("Data type is different", mock.getDataType(), returnedData.get(AccessRight.DATA_TYPE));
    assertEquals("Description is different", mock.getDescr(), returnedData.get(AccessRight.DESCRIPTION));
    assertEquals("Feature is different", mock.getFeature(), returnedData.get(AccessRight.FEATURE));
    assertEquals("Role is different", mock.getRoleUID(), returnedData.get(AccessRight.ROLE));
  }

  private void checkGetAccessRightFail(
    IEvent event, String session, StateMachine sm, boolean eventEx)
  {
    ...
    {
      ACLLogger.infoLog(CLASSNAME, "checkGetAccessRightFail", "Fail due to event exception: "+ex.getMessage());
      if (!eventEx)
        assertTrue("Unexpected Event Exception", false);
      ACLLogger.infoLog(CLASSNAME, "checkGetAccessRightFail", "Exit");
      return;
    }
    catch (Exception ex)
    {
      ACLLogger.errorLog(CLASSNAME, "checkGetAccessRightFail", "Error Exit", ex);
      assertTrue("Event Exception", false);
    }
    assertNotNull("Response is null", response);
    assertTrue("Event status is incorrect", !response.isEventSuccessful());
    assertEquals("Msg code incorrect", IErrorCode.FIND_ENTITY_BY_KEY_ERROR, response.getMessageCode());
...
//  }
  ...
  private void prepareTestData() throws Exception
  {
    ...
    _events = new GetAccessRightEvent[]
    {
      //0-1 used in modifyRejectedByAction
      createEvent(ACR_UID_1),
      createEvent(ACR_UID_DUMMY),

      //2 used in modifyAcceptedByManager
      createEvent(ACR_UID_1),
    };
  ...
  {
    ACR_UID_1 = createTestAccessRight(DESC_1, ROLE_UID_1, FEATURE_1, ACTION_1, DATA_TYPE_1, CRITERIA_2);
    _mockAcrs = new AccessRight[]
                {
                  createMockAccessRight(ACR_UID_1, DESC_1, ROLE_UID_1, FEATURE_1, ACTION_1, DATA_TYPE_1, CRITERIA_2),
                };
  }
  ...
  private GetAccessRightEvent createEvent(Long acrUId)
    throws Exception
  {
    return new GetAccessRightEvent(acrUId);
  }
