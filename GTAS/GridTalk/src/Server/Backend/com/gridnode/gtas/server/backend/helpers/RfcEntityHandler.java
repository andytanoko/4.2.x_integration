/**
 * This software is the proprietary information of GridNode Pte Ltd.
 * Use is subjected to license terms.
 *
 * Copyright 2001-2002 (C) GridNode Pte Ltd. All Rights Reserved.
 *
 * File: RfcEntityHandler.java
 *
 ****************************************************************************
 * Date           Author              Changes
 ****************************************************************************
 * Dec 17 2002    Koh Han Sing        Created
 * Oct 31 2005    Neo Sok Lay         1. Remove implementation for getHome()
 *                                    2. Implement getHomeInterfaceClass()
 */
package com.gridnode.gtas.server.backend.helpers;

import java.util.Collection;

import com.gridnode.gtas.server.backend.entities.ejb.IRfcLocalHome;
import com.gridnode.gtas.server.backend.entities.ejb.IRfcLocalObj;
import com.gridnode.gtas.server.backend.model.Rfc;
import com.gridnode.pdip.framework.db.EntityHandlerFactory;
import com.gridnode.pdip.framework.db.LocalEntityHandler;
import com.gridnode.pdip.framework.db.filter.DataFilterImpl;
import com.gridnode.pdip.framework.util.ServiceLocator;

/**
 * This helper class helps to handle the calls to the Home, Local interfaces
 * of the RfcBean.
 *
 * @author Koh Han Sing
 *
 * @version 2.0
 * @since 2.0
 */
public final class RfcEntityHandler
  extends          LocalEntityHandler
{
  private RfcEntityHandler()
  {
    super(Rfc.ENTITY_NAME);
  }

  /**
   * Get an instance of a RfcEntityHandler.
   */
  public static RfcEntityHandler getInstance()
  {
    RfcEntityHandler handler = null;

    if (EntityHandlerFactory.hasEntityHandlerFor(Rfc.ENTITY_NAME, true))
    {
      handler = (RfcEntityHandler)EntityHandlerFactory.getHandlerFor(
                  Rfc.ENTITY_NAME, true);
    }
    else
    {
      handler = new RfcEntityHandler();
      EntityHandlerFactory.putEntityHandler(Rfc.ENTITY_NAME,
         true, handler);
    }

    return handler;
  }

  /**
   * Find the Rfc whose name is the specified.
   *
   * @param rfcName The name of the Rfc.
   * @return the Rfc having the specified rfc name.
   */
  public Rfc findByRfcName(String rfcName)
    throws Throwable
  {
    DataFilterImpl filter = new DataFilterImpl();
    filter.addSingleFilter(
      null,
      Rfc.NAME,
      filter.getEqualOperator(),
      rfcName,
      false);

    Collection result = getEntityByFilterForReadOnly(filter);
    if(result == null || result.isEmpty())
      return null;
    return (Rfc)result.iterator().next();
  }

  /**
   * Looks up the Home interface in the local context.
   *
   * @return The Local Home interface object.
   *//*
  protected Object getHome() throws java.lang.Exception
  {
    return ServiceLocator.instance(ServiceLocator.LOCAL_CONTEXT).getHome(
      IRfcLocalHome.class.getName(),
      IRfcLocalHome.class);
  }*/

  
  protected Class getHomeInterfaceClass() throws Exception
	{
		return IRfcLocalHome.class;
	}

	/**
   * This method should return the Proxy (Remote/Local) interface class for the
   * EntityBean it handles.
   *
   * @return The Proxy interface class for the EntityBean.
   */
  protected Class getProxyInterfaceClass() throws Exception
  {
    return IRfcLocalObj.class;
  }
}