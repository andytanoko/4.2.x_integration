...
import com.gridnode.gtas.events.gridform.LoadFormEvent;
...
/**
 * Action that handles the loading of a form object.
 *
 * @version 2.0
 * @since 2.0
 * @author Jared Low
 */
public class LoadFormAction extends GridTalkEJBAction
{
  public IEventResponse perform(IEvent event) throws EventException
  {
    ...
    try
    {
      ...
      LoadFormEvent loadFormEvent = (LoadFormEvent)event;
      GFForm form = null;
      if (loadFormEvent == null)
      {
        throw new EventException("Event is null");
      }

      // Request to load form.
      form = mgr.loadForm(loadFormEvent.getXMLFilename());
  ...
  private IEventResponse constructEventResponse(NestingException ex)
  {
    ...
    response = new BasicEventResponse(
                   IErrorCode.LOAD_FORM_ERROR,
                   new Object[]{},
                   (short)-1,
                   ex.getLocalizedMessage(),
                   ex.getStackTraceString());

    Logger.err("[LoadFormAction.perform] Event Error", ex);
    return response;
  }
