 ...
 * 2006-04-24     Neo Sok Lay         Init the noP2P and noUDDI options based on values in
 *                                    action mapping.
 */
...
public class LoginAction extends GTActionBase
{
...
//  sumedh : 21 Nov 05
      ...
      if(!failed)
      {
        try
        {
          ...
          { //20031105AH - HACK for GNDB00016109
             ...
             //NSL2060424 - Flag for hiding P2P/UDDI functions
             gtasSession.setNoP2P(((LoginActionMapping)mapping).isNoP2P());
             gtasSession.setNoUDDI(((LoginActionMapping)mapping).isNoUDDI());
          }
  ...
  //sumedh : 21 Nov 05
  private void addNwForwardToSession(HttpServletRequest request)
  {
  	log("addNwForwardToSession BEGINS");
  	String nwForward = request.getParameter("nw_forward");
    log("nwForward != null : " + (nwForward != null));
    if (nwForward != null)
    {
    	HttpSession session = request.getSession();
    	session.setAttribute("nw_forward", nwForward);
    }
  }
  ...
  {
  	com.gridnode.gtas.client.netweaver.helper.Logger.debug("[LoginAction]" + message);
  }
}
