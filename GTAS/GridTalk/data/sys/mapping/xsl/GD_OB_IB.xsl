<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="GridDocument">
      ...
      <xsl:if test="count(GdocId)!=0">
			  <SenderGdocId><xsl:value-of select="GdocId"/></SenderGdocId>
      </xsl:if>
			<SenderRoute><xsl:value-of select="SenderRoute"/></SenderRoute>
			<SenderPartnerFunction><xsl:value-of select="PartnerFunction"/></SenderPartnerFunction>
      ...
      <xsl:if test="count(RnProfileUid)!=0">
	    ...
	    <xsl:if test="count(UserTrackingID)!=0">
	    ...
	    <xsl:if test="count(ProcessInstanceUid)!=0">
			...
			<xsl:if test="count(SenderCert)!=0">
				<SenderCert><xsl:value-of select="SenderCert"/></SenderCert>
			</xsl:if>
			
			<xsl:if test="count(ReceiverCert)!=0">
				<ReceiverCert><xsl:value-of select="ReceiverCert"/></ReceiverCert>
			</xsl:if>
			<isOriginalDoc><xsl:value-of select="isOriginalDoc"/></isOriginalDoc>
