...
//  }
  ...
  public static void store(String pathKey, String subPath, byte[] blockContent,
        String fileId, int blockNo)
        throws FileAccessException
  {
    String filename = getBlockFileName(fileId, blockNo);
    if (FileHelper.exist(pathKey, subPath, filename))
      if (FileHelper.length(pathKey, subPath, filename) != (long) blockContent.length)
   ...
   */
  public static File[] getOriginalFiles(String pathKey, String subPath, String[] filenames,
        long[] size, String fileId, boolean isZip)
    ...
    try
    {
      ...
      for (int i = 0; i < newFile.length; i++)
      {
        ...
        else
        {
          ...
          newFile[i] = FileHelper.createNewLocalFile(pathKey, subPath, filenames[i]);
          if (newFile[i] == null)
            throw new IOException("Unable to create empty file " + filenames[i]);
          if (size[i] < 1)
  ...
  private static byte[] getBlockContent(String pathKey, String subPath, String fileId, int blockNo)
        throws FileAccessException
  {
   ...
   * @exception       ApplicationException thrown when error occurs.
   */
  public static File[] getBlockFiles(String pathKey, String subPath, String fileId, boolean isZip)
        throws FileAccessException, ApplicationException
  {
    return null;
  }
