...
//  private boolean _isZip = false;
   ...
   */
  public static int storeFilesIntoBlocks(String pathKey, String subPath, File[] files,
          int blockSize, int totalBlocks, int lastBlockSize, boolean isZip, String fileId)
    ...
    try
    {
      ...
      for (int currentBlock = 0; currentBlock < totalBlocks; currentBlock++)
      {
        ...
        while (remainingSizeToRead > 0 && currentFileIndex < files.length)
        {
          ...
          {
            if (files[currentFileIndex] == null ||   //File is null or
                !files[currentFileIndex].exists() || //File does not exist or
                files[currentFileIndex].length() < 1 //File length is 0 or less
               )
            {
              PackagingLogger.debugLog(CLASS_NAME,
                      "storeFilesIntoBlock",
                      "Skip file: " + ((files[currentFileIndex] == null)? (""+currentFileIndex) : files[currentFileIndex].getAbsolutePath()));
            ...
            else
            {
              PackagingLogger.debugLog(CLASS_NAME,
                      "storeFilesIntoBlock",
                      "Open file to read: " + files[currentFileIndex].getAbsolutePath());
          ...
          {
            int bufferSize = (remainingSizeToRead > BUFFER_SIZE) ? BUFFER_SIZE : remainingSizeToRead;
          ...
          if (currentFilePointer >= files[currentFileIndex].length()) //more to read => reach eof
          {
        ...
        if (currentFileIndex <= files.length)
        {
          ...
          storeBlock(pathKey, subPath, currentBlock, new ByteArrayInputStream(baos.toByteArray()), fileId);
        }
        else
  ...
  private static void storeBlock(String pathKey, String subPath, int blockNo,
          InputStream blockData, String fileId) throws FileAccessException
  {
    FileHelper.create(pathKey, subPath, getBlockFileName(fileId, blockNo), blockData, true);
  }

  public static int getTotalBlocks(File[] files, int blockSize, int thresholdSize)
  {
  ...
  public static int getLastBlockSize(File[] files, int blockSize, int thresholdSize)
  {
    long totalFileSize = 0;
    for (int i = 0; i < files.length; i++)
    {
    ...
    int lastBlockSize = (int)(totalFileSize % blockSize);
    if ((blockSize + lastBlockSize) <= thresholdSize)
      lastBlockSize += blockSize;// combine last block together with the previous block.
    return lastBlockSize;
  }

  public static byte[] getBlock(String pathKey, String subPath, String filename)
          throws FileAccessException
  {
    return FileHelper.getFileContent(pathKey,
                                     subPath,
                                     filename
                                    );
  }

  public static boolean checkFilesStoredIntoBlocks(String pathKey, String subPath, File[] files,
          int totalBlock, boolean isZip, String fileId) throws FileAccessException
  {
  ...
  public static boolean checkBlockExist(String pathKey, String subPath, String filename)
          throws FileAccessException
  {
    return FileHelper.exist(pathKey,
                            subPath,
                            filename
                           );
  }
