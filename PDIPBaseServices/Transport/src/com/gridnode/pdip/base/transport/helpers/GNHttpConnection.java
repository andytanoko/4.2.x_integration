 ...
 * Oct 17 2005    Neo Sok Lay             Change auth_username and auth_password
 *                                        to protected access to improve performance.
 * Jan 27 2006    Neo Sok Lay             Remove setting Sun JCE provider in System Property.
 * Jan 12 2007    Neo Sok Lay             To use ProxySelector for proxy connections.
...
class TimedURLConnection
{
  ...
  public static OutputStream getOutputStream(URLConnection urlconnection, int timeout)
    throws IOException
  {
    Object event = new Object();
    OutputStreamThread outputstreamthread = new OutputStreamThread(urlconnection, event);
  ...
  public static InputStream getInputStream(URLConnection urlconnection, int timeout)
      throws IOException
  {

    Object event = new Object();
    InputStreamThread inputstreamthread = new InputStreamThread(urlconnection, event);
...
public class GNHttpConnection
{
  ...
  /*NSL20070112
  private Object newProxySetting(boolean success, boolean usingProxy, String ip, String port)
  {
  ...
  */
  ...
  public boolean testingVMProxy()
    {
      HttpMessageContext context = HttpMessageContext.getInstance();
        context.Info("HttpMessageContext[testingVMProxy] Testing VM Proxy Support ");
        if(vm_proxy_support_tested)
        {
          context.Info("HttpMessageContext[testingVMProxy] VM Proxy Support Tested Already " + vm_proxy_support);
        ...
        try
        {
          ...
          GNHttpConnection connection = new GNHttpConnection("http://www.gridnode.com/", 10);
  ...
  static public void setHttp_proxy(String http_proxy_pac, String http_proxy_url, String http_proxy_port, String http_proxy_user, String http_proxy_passwd)
  {
    clearProxySettings();
    ...
    do
    {
      ...
      if(index >= 0)
      {
        ...
        if(pacproxy.indexOf("DIRECT") == -1)
        {
          ...
          if(proxyList.indexOf(pacproxy) < 0)
            proxyList.add(pacproxy);
        }
      }
    }while(index >= 0);
    return proxyList;
  ...
  //1 not config
  {
    ...
    {
    if(getHttp_proxy_pac().length() <= 0 && (getHttp_proxy_url().length() <= 0 || getHttp_proxy_port().length() <= 0))
        {
    ...
    if(!testingVMProxy())
       {
        context.Debug("GNHttpConnection[prepareProxy] Cannot Support Proxy for this JVM: " + context.getJava_home());
        return -1;
       }
    try
    {
      if(getHttp_proxy_pac().length() > 0)
      {
        ...
        if(!retrievedPac)
        {
          context.Debug("GNHttpConnection[prepareProxy] retrieving pac script from " + getHttp_proxy_pac());
          GNHttpConnection connection = new GNHttpConnection(getHttp_proxy_pac(), 5);
...
class AuthImpl extends Authenticator
{
   protected PasswordAuthentication getPasswordAuthentication()
   {
      if(auth_username != null && auth_username.length() > 0)
         return new PasswordAuthentication(auth_username, auth_password.toCharArray());
      else
         return null;
   }
 }
*/
  ...
  public String logMessageSetting()
  {
      ...
      messageConfig   += "   URL[" + url + "]\r\n" +
                         ...
                         //"   Proxy Setting[" + auth_username + ":" + auth_password + "@" + getConfig_http_proxy_url() + ":" + getConfig_http_proxy_port() + "]\r\n" +
                         "   Timeout["+ timeout + "]\r\n";
  ...
  {
    tsFile = JavaKeyStoreHandler.getTrustStoreName(tsFile);
    tsPass = JavaKeyStoreHandler.getTrustStorePassword(tsPass);
  ...
  private byte[] sendHttpMessage(byte[] body) throws IOException
  {
    ...
    context.Debug("GNHttpConnection [sendHttpMessage] TimedURLConnection.getInputStream timeout[" + timeout + " S]");
  ...
  public byte[] doGetPOST(Hashtable headers, byte[] content) throws Exception
  {
    ...
    {
      context.Debug("GNHttpConnection[doGetPOST] Connecton fail for " + url + "\r\n" + context.Exception2String(ex));
      throw ex;
    }
  }
  ...
  public byte[] doGetPOST(Hashtable headers, byte[] content) throws Exception
  {
    ...
    try
    {
      ...
      {
          context.Debug("GNHttpConnection[doGetPOST] use cached proxy setting for " + url);
      ...
      synchronized(context)
      {
        proxysettingList.put(url, newProxySetting(true, isUsingProxy(), getConfig_http_proxy_url(),getConfig_http_proxy_port()));
      }
      return getResponseMessage();
    }
    catch (Exception ex)
    {
        ...
        if(isUsingProxy())
        {
            context.Debug("GNHttpConnection[doGetPOST] Cached Proxy connecton fail for " + url + "\r\n" + context.Exception2String(ex));
        }
        else
            context.Debug("GNHttpConnection[doGetPOST] Direct connecton fail for " + url + "\r\n" + context.Exception2String(ex));
    }

    if(isUsingProxy())//try direct
    {
        context.Debug("GNHttpConnection[doGetPOST] try Direct connection for " + url);
        try
        {
            setUsingProxy(false);//direct
            connect();
    ...
    for(int i = 0; i < pacProxyList.size(); i++)
      {
        ...
        context.Debug("GNHttpConnection[doGetPOST] try one proxy from list[" + getAuth_username() + ":" + getAuth_password() +  "@" + proxy_IP + ":" + proxy_Port + "]");
        try
        {
          connect();
          sendHttpMessage(headers, content);
        ...
        {
          hasex = ex;
          context.Debug("GNHttpConnection[doGetPOST] try proxy connecton fail for " + url + "\r\n" + context.Exception2String(ex));
        }
      }
    if(hasex == null)
  ...
  */
  ...
  public byte[] doGet(Hashtable headers) throws Exception
  {
      setDoGet();
  ...
  public byte[] doPost(Hashtable headers, byte[] body) throws Exception
  {
      setDoPost();
