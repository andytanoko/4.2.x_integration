...
final class GNProxySelector extends ProxySelector
{
  ...
  GNProxySelector(ProxySelector def, Properties setting)
  {
    ...
    if (defsel != null)
    {
      try
      {
        ...
        for (Proxy p : l)
        {
          ...
          {
            log("Adding " + p + " to default proxy list");
            defProxyList.add(p);
          }
        }
      }
    ...
    while (st.hasMoreTokens())
    {
      ...
      String port = setting.getProperty("proxy." + s + ".port", "80").trim();
      ...
      log("Adding " + i + " to custom proxy list");
    }
...
//    debug("Adding default proxies: " + defProxyList);
      l.addAll(defProxyList);
      for (InnerProxy p : ps)
      {
        if (p.isdDown())
        {
          ...
          { // ok, time is up
            log("Putting " + p.toProxy() + " back to action");
            p.setDowntime(0); // reset, enable this proxy
          }
          else
          {
            l2.add(p.toProxy());  // add to disabled list
            continue; // Ignore this one first, wait for retry timeout
          }
        }

//      debug("Adding " + p.toProxy());
        l.add(p.toProxy());
      }
  ...
  public void connectFailed(URI uri, SocketAddress sa, IOException ioe)
  {
    ...
    if (p != null)
    {
      error("Proxy failed: " + p.proxy.toString() + " at " + new Date());
  ...
  /*
      ...
      try
      {
        FileInputStream fin = new FileInputStream("proxies.properties");
        setting.load(fin);
      }
      catch (Exception ex)
      {
      ...
      GNProxySelector.install(setting);
       }*/
}
